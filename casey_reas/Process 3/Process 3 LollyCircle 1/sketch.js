...
function draw() {
  ...
  circleRadiusToBaseRatio = map(slider2WithState.sliderEl.value(), 0, 100, 1, 8);
  signalTouches();
}
  ...
  for (let i = 0; i < elements.length - 1; i++) {
    ...
    for (let j = i + 1; j < elements.length; j++) {
      ...
      if (touchingForms(iCircle, jCircle)) {
        ...
        const rad = w * circleRadiusToBaseRatio;
        noFill();
        stroke(myHue(i, j), saturation, lum, alphaHere);
        line(0, 0, 0, totalLen);
        circle(0, totalLen, rad);
        pop();
