...
const manySegments = 30;
 
let elements;
 
 
function myManySegs(x,y, w, h) {
  return floor(map(x*y, 0, w * h, 0, manySegments)) + 1;
}
  ...
  for (let i = 0; i < elements.length - 1; i++) {
    ...
    for (let j = i + 1; j < elements.length; j++) {
      ...
      if (touchingForms(iCircle, jCircle)) {
        ...
        const manySegs = myManySegs(midPoint.x,midPoint.y, width, height);
