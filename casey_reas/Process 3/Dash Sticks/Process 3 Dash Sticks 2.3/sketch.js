...
const manySegments = 20;
 
let elements;

function myManySegs(i,j) {
  return ((i*7 + j*11) % manySegments) + 1;
}
  ...
  for (let i = 0; i < elements.length - 1; i++) {
    ...
    for (let j = i + 1; j < elements.length; j++) {
      ...
      if (touchingForms(iCircle, jCircle)) {
        ...
        const manySegs = myManySegs(i,j);
        const segLen = totalLen/manySegs/2;
        for (let i = 0; i < 2*manySegs; i++) {
          const col = i % 2 == 0
            ? color(myHue2(i, j), saturation, lum, alphaHere)
            : color(0,0,0,0);
          stroke(col);
