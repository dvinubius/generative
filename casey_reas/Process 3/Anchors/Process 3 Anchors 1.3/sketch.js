...
function setup() {
  ...
  sliderWithState = createSliderWithState(0.5);
}
  ...
  for (let i = 0; i < elements.length - 1; i++) {
    ...
    for (let j = i + 1; j < elements.length; j++) {
      ...
      if (touchingForms(iCircle, jCircle)) {
        ...
        // const alphaHere = map(distIJ, 1, 0, 0, alpha); // original with inverted alpha
        ...
        strokeWeight(1);
        beginShape();
        curveVertex(iCircle.x, iCircle.y);
        curveVertex(iCircle.x, iCircle.y);
        curveVertex(anchorPt.x, anchorPt.y);
        curveVertex(anchorPt.x, anchorPt.y); 
        curveVertex(jCircle.x, jCircle.y);
        curveVertex(jCircle.x, jCircle.y);
        endShape();
          
        stroke(myHue(i, j), saturation, lum, 0.2);
        strokeWeight(0.3);
        iCircle.drawCenter();
        jCircle.drawCenter();
